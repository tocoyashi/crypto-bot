...
# ✨ Changed to 15 minutes for more signals
TIMEFRAME = "15m"
...
<i>Automated signal. Trade responsibly.</i>
    ...
    try:
        ...
        print(f"Error sending {coin_name} signal")
    ...
    for symbol in SYMBOLS:
        try:
            ...
            if ema_buy or macd_buy:
                ...
                send_crypto_signal(symbol, "LONG", strategy_name, str(entry), "10", str(tp1), str(tp2), str(sl), img)
                time.sleep(2)
                
            # Check for ANY sell signal
                ...
                send_crypto_signal(symbol, "SHORT", strategy_name, str(entry), "10", str(tp1), str(tp2), str(sl), img)
                time.sleep(2)
            else:
